...
namespace Hdc.Mv.Halcon
{
    public static class HalconExtensions
    {
        ...
        public static HImage ToHImage8Bpp(this ImageInfo imageInfo)
        {
            var hImage = new HImage("byte", imageInfo.PixelWidth, imageInfo.PixelHeight, imageInfo.BufferPtr);
            return hImage;
        }
...
/*            int bufferSize = stride * pixelHeight;
        ...
        public static Task<BitmapSource> ToBitmapSourceAsync(this HImage hImage)
        {
            return Task.Run(() => hImage.ToBitmapSource());
        }
        ...
        public static HImage MirrorAndCrop(this HImage image, bool mirrorRow, bool mirrorColumn,
            int x, int y, int width, int height)
        {
            HImage finalImage = image;
            ...
            {
                var croppedImage = finalImage.Crop(x, y, width, height);
                finalImage.Dispose();
        ...
        public static HImage Crop(this HImage image, int x, int y, int width, int height)
        {
            int width2 = width;
            int height2 = height;
            ...
            var croppedImage = image.CropPart(y, x, width2, height2);
            return croppedImage;
        }
    }
}